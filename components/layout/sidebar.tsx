...
/** Fixed 16×16 slot so wordmarks and symbols share the same left edge. */
function NavIconSlot({
  ...
  return (
    <span
      className={`flex h-4 w-4 shrink-0 items-center justify-start overflow-visible text-current ${className ?? ""}`}
    >
      {children}
    </span>
  );
  ...
  return (
    <div className="space-y-0.5">
      <div className="flex items-stretch gap-0.5 rounded-lg">
        <Link
          href={item.href}
          className={`${base} min-w-0 flex-1 items-center justify-start text-left ${
            parentActive ? activeClass : inactiveClass
          }`}
        >
          <NavIconSlot>{icons[item.icon]}</NavIconSlot>
    ...
    <div className="space-y-0.5">
      <div className="flex items-stretch gap-0.5 rounded-lg">
        <button
          ...
          onClick={onToggleExpanded}
          aria-expanded={expanded}
          className={`${base} min-w-0 flex-1 items-center justify-start text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--accent)/20 ${
            sectionActive ? activeClass : inactiveClass
          }`}
        >
          <NavIconSlot>{icons[headerItem.icon]}</NavIconSlot>
