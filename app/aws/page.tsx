      ...
      <header className="relative overflow-hidden rounded-2xl border border-(--border-hairline) bg-(--bg-elevated) px-6 py-8 shadow-sm sm:px-8">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-(--text-secondary)">{lead}</p>
            <p className="mt-4 text-sm text-(--text-muted)">
              Active profile and default region come from{" "}
              <Link href="/settings" className="font-medium text-(--accent) hover:underline">
                Settings
              </Link>
              . Under the hood, that uses the same named profiles as your local{" "}
              ...
              and config (set up with{" "}
              <code className="rounded border border-(--border-hairline) bg-(--accent-muted)/50 px-1.5 py-0.5 font-(family-name:--font-mono) text-[12px] text-(--text-primary)">
                aws configure
              </code>{" "}
              or your usual workflow).
            </p>
          </div>
          <div
            className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(145deg,var(--accent-muted),var(--success-muted))] shadow-[inset_0_1px_0_0_rgba(35,47,62,0.12)] sm:mt-6"
            aria-hidden
          >
            {/** Same mark as `icons.aws` in `components/layout/sidebar.tsx` */}
            <svg width="32" height="32" viewBox="0 0 24 24" className="shrink-0" aria-hidden>
              <path
                fill="#232F3E"
