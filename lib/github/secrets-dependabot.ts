...
export async function fetchOrgDependabotSecrets(org: string): Promise<GHSecretInventoryRow[]> {
  try {
    const data = await githubFetch<{ secrets: RawSecret[] }>(
      `/orgs/${encodeURIComponent(org)}/dependabot/secrets?per_page=100`,
    );
    return (data.secrets ?? []).map((s) => ({
      name: s.name,
      kind: "secret" as const,
      scope: "organization" as const,
      platform: "dependabot" as const,
      ownerLogin: org,
...
export async function fetchRepoDependabotSecrets(
  repoFullName: string,
  ownerLogin: string,
  ...
  try {
    const data = await githubFetch<{ secrets: RawSecret[] }>(
      `/repos/${path}/dependabot/secrets?per_page=100`,
    );
    return (data.secrets ?? []).map((s) => ({
      name: s.name,
      kind: "secret" as const,
