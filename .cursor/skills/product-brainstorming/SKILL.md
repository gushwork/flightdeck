...
description: Brainstorm product ideas, explore problem spaces, and challenge assumptions. Includes Flightdeck local-first / Electron-like product DNA — credentials and trust stay on the machine; development always uses locally stored credentials for any service integration.
---
...
## Flightdeck reference concepts (aws-manager)

Use this block when brainstorming features for **Flightdeck** — a local-first ops console that combines **AWS** (secrets, Amplify, IAM, audit) with **GitHub Actions** and room for more **utilities**, unified under one shell. Align with the GitHub Actions module plan (`.cursor/plans/github_actions_module_plan_834a6adb.plan.md` or `CODEBASE.md` GitHub section) when discussing parity with shipped UX.

### Local app model (think: Electron, even when it’s a browser)

**Treat Flightdeck like a local desktop app** — the browser window is just the shell; the **source of truth for identity and secrets is the operator’s machine**, not our servers.

- **Mental model**: Same as **Electron** — a main/trusted process (here: Next.js **server** + API routes, or future Electron `main`) reads credentials and talks to AWS/GitHub/etc.; the **renderer** (React UI) never holds long-lived secrets, only session UI state.
- **No “paste your production API key into our cloud”** as the default story. Prefer: existing **CLI sessions**, **OS keychains**, **`~/.aws/credentials`**, **`gh auth`**, **`git` remote identity**, and **`.env.local`** for **non-secret** dev config only (URLs, feature flags) — never ship real credentials in repo or UI storage for development.
- **Development rule of thumb**: When designing or brainstorming any new integration (another cloud, SaaS, registry), ask: **“How does a dev run this authenticated on localhost without putting secrets in git or in a remote DB?”** If the answer doesn’t start with something local (profile, token file, CLI OAuth), push back — that’s a different product (hosted SaaS), not this one.
- **User trust copy**: “Runs on your machine with your existing tools” beats “secure vault in our cloud” for this persona.

### Positioning and jobs-to-be-done
- **Job**: “I’m on call / shipping — show me what’s broken or running, let me act, don’t make me context-switch across five tabs.”
- **Multi-surface, single layout**: Same chrome (sidebar, topbar, agent) whether the user is in Secrets, GitHub, or IAM utilities — predictability beats novelty.
- **Name story** (examples from plan): *Flightdeck* (cockpit / monitoring), *Launchpad* (hub), *Opsconsole* (neutral), *CommandPost* (single pane of glass). Use naming to align tone: calm control vs. playful hub.

### Authentication and trust (always local credentials)

**Non-negotiable for Flightdeck brainstorming:** credentials for **development and normal operation** flow from **locally stored** material — CLIs, named AWS profiles, environment files on disk — not from a first-party hosted credential store unless explicitly scoped as a future SaaS fork.

- **GitHub**: Prefer **`gh auth token`** (and `gh api`) over asking users to paste PATs into Settings. The browser calls `/api/github/*`; the server shells out or reads the same token path `gh` uses.
- **AWS**: **`AWS_PROFILE` / `~/.aws/credentials`** / `fromIni` — profile picker + region; no shared “Flightdeck account” in the cloud for AWS access.
- **Other services** (future): Same bar — **Kubeconfig**, **gcloud config**, **Docker credential helpers**, etc. Brainstorm features that **delegate to those tools** or read their files **only server-side**, never bundle secrets into the client bundle.
- **`.env.local`**: OK for **OpenRouter keys, `NEXT_PUBLIC_*`, DB URL for optional local cache** — treat as **machine-local dev config**; document that production-like deploys should use the same pattern (env on host), not committed secrets.
- **Contrast for ideation**: If someone proposes “users upload API keys to our backend,” counter with: “Electron app would store that in OS keychain — how do we stay equivalent with local-only storage?”

### Information architecture
- **Module registry** drives nav and feature flags — new areas (e.g. GitHub, future utilities) ship as sections, not one-off routes scattered in the shell.
- **Summary first, then density**: e.g. GitHub **Actions** dashboard (health, active, failures) → **All runs** grid → per-run actions — matches progressive disclosure in the plan.
- **Utilities as a pattern**: “Utility” isn’t a junk drawer — it’s IAM-style tools today; tomorrow could be more CLIs (e.g. kubectl, terraform) with the same UX contract: **fast, keyboard-first, confirm destructive work.**

### GitHub Actions product ideas (from plan DNA)
- **Smart polling**: Faster intervals while something is `in_progress` or recently failed; slower when green; pause when the tab is backgrounded — respects attention and API limits.
- **Nav signal without noise**: A small **activity indicator** on GitHub nav when runs are live — urgency only when something is actually moving.
- **Full control surface**: Re-run (all / failed), cancel, dispatch, view logs / GitHub — product debates: which actions are safe without confirm, which need audit trails later.
- **Keyboard-first as a differentiator**: Power users live in the terminal; the UI should feel like an extension (`/` search, arrows in grids, shortcuts in copy on screen).
- **Shareable filtered views**: URL-encoded filters so engineers can paste “all failures on repo X” in Slack — distribution mechanism for the product.
- **Performance as UX**: Capping repos/runs and time-budgeting API work so the dashboard **always** returns in seconds, not tens of seconds — slow empty states kill trust.

### Future-facing questions (for sessions)
- Org-wide or “starred repos only” scopes when listing Actions across many repos?
- Workflow enable/disable from the UI vs. policy-as-code purists?
- Unified “inbox” of failures across AWS alarms + GitHub + future sources?

When brainstorming Flightdeck features, connect ideas back to: **calm when healthy**, **loud when action needed**, **local / Electron-like trust** (credentials never leave the operator’s machine for auth), **keyboard speed**, **no credential theater**, and **every new service integration defaults to locally stored dev credentials**.
